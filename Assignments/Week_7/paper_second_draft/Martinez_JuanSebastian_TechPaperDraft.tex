...
\centering
    \begin{subfigure}{0.45\textwidth}
        ...
        \caption{First Pareto front for 2 initial observations}
        \label{li_intial}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\textwidth]{hi_ex1_00}
        \caption{First Pareto front for 10 initial observations}
        \label{hi_intial}
    \end{subfigure}
\caption{Initial Pareto fronts for the synthetic examples: a) First Pareto front with just 2 initial observations. b) First Pareto front with 10 initial observations}
\label{initial_obs}
\end{figure}

Figure \ref{initial_obs} shows the initial Pareto fronts generated by the algorithm for the two cases in the first iteration. In the figure, the rhombus represent the points selected to be part of the the front, the red point is the point added to the front in that specific iteration and the bold line is what is estimated as the true Pareto front of the problem.\\
...
\centering
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \label{hi_final}
    \end{subfigure}
\caption{Initial Pareto fronts for the synthetic examples: a) Final Pareto front with just 2 initial observations after 20 iterations. b) Final Pareto front with 10 initial observations after 10 iterations}
\label{final_front}
\end{figure}

Figure \ref{final_front} shows the final state of the program after the fixed number of iterations on both cases. Figure \ref{li_final} shows the constructed Pareto front after 20 iterations and figure \ref{hi_final} illustrates the Pareto front after 10 iterations. The first case corresponds to only 2 initial observations and the later is the case of 10 observations.

\section{Discussion}

Figure \ref{li_initial} shows how few initial observations generate a not very significant Pareto front, although its points are near the true curve. On the other hand, figure \ref{hi_intial} shows that a bigger amount of observations generate a more accurate set up.\\
